...
const Dashboard = () => {
  ...
  // --- AUTO CALCULATION LOGIC (Fixes the 0 issue) ---
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        toast.success("Saved to MOBILE CITY!");
      ...
      <main className="p-4 md:p-6 max-w-[1500px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-white p-5 rounded-2xl shadow-sm border-l-4 border-l-blue-500 flex justify-between items-center">
            <div><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Customers</p><h3 className="text-xl font-black text-blue-600">{salesRecords.length}</h3></div>
          ...
          {activeTab !== 'Bill' ? (
            <>
              <div className="p-4 border-b border-slate-100 flex flex-col md:flex-row gap-4 justify-between items-center">
                <div className="relative w-full md:w-1/2">
                  ...
                  <input type="text" placeholder="Search by name, cnic, imei, model..." className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-teal-500 outline-none text-sm transition-all" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {loading ? (
                      <tr><td colSpan={10} className="px-4 py-12 text-center text-slate-300 animate-pulse text-sm uppercase">Loading Mobile City Data...</td></tr>
                    ) : filteredData.length > 0 ? (
                      filteredData.map((c) => (
                        <tr key={c._id} className="hover:bg-slate-50/80 transition-all group">
                          <td className="px-4 py-4 font-bold text-slate-800 text-sm">{c.name}</td>
                          <td className="px-4 py-4 text-xs font-bold text-slate-600">{c.phone}</td>
                          <td className="px-4 py-4 text-[11px] font-mono text-slate-400">{c.cnic}</td>
                          <td className="px-4 py-4 text-xs font-bold text-slate-500 uppercase">{c.model}</td>
                          <td className="px-4 py-4 text-center"><span className={`px-2 py-1 rounded text-[9px] font-bold border uppercase ${c.type === 'Purchase' ? 'bg-orange-50 text-orange-600 border-orange-200' : 'bg-teal-50 text-teal-600 border-teal-200'}`}>{c.emi}</span></td>
                          ...
                          <td className="px-4 py-4 text-xs text-slate-400 font-medium">{c.date}</td>
                      ...
                      <tr><td colSpan={10} className="px-4 py-20 text-center text-slate-300 font-medium text-xs">NO RECORDS IN MOBILE CITY DATABASE.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            <div className="p-6 md:p-10 max-w-5xl mx-auto">
              <div className="mb-8 text-center uppercase tracking-tight">
                <h3 className="text-2xl md:text-3xl font-black text-slate-800">MOBILE CITY INVOICE FORM</h3>
                <p className="text-slate-400 font-bold text-[10px] tracking-widest mt-1">Create or Update Record</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="flex justify-center gap-2 p-1.5 bg-slate-100 rounded-xl w-fit mx-auto border border-slate-200 shadow-inner mb-6">
                  ...
                  <button type="button" onClick={() => setFormData(prev => ({...prev, type: 'Purchase'}))} className={`px-12 py-2.5 rounded-lg font-bold text-xs transition-all ${formData.type === 'Purchase' ? 'bg-white text-orange-500 shadow-sm' : 'text-slate-400'}`}>PURCHASE</button>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50/50 p-6 rounded-3xl border border-slate-100">
                  <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase ml-2">Customer Name</label>
                  <input required className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500 outline-none font-bold text-sm" placeholder="Enter Name" value={formData.name} onChange={e => setFormData(prev => ({...prev, name: e.target.value}))} /></div>
                  
                  <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase ml-2">Phone Number</label>
                  ...
                  <div className="space-y-1.5 md:col-span-2"><label className="text-[10px] font-bold text-slate-400 uppercase ml-2">Mobile Model</label>
                  ...
                  <div className={`${formData.type === 'Sale' ? 'col-span-1' : 'col-span-1 md:col-span-2'}`}>
                    <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase ml-2">Total Price (Rs)</label>
                  ...
                  {formData.type === 'Sale' && (
                    <>
                      <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase ml-2">Cash Paid (Rs)</label>
                      ...
                      <div className="space-y-1.5 md:col-span-2"><label className="text-[10px] font-bold text-slate-400 uppercase ml-2">Remaining Balance (Rs)</label>
                      <input readOnly className="w-full px-4 py-3 bg-red-50 border border-slate-100 rounded-xl font-black text-red-500 text-lg outline-none cursor-not-allowed" value={formData.remainingAmount} /></div>
                    </>
                  )}

                  <div className="space-y-1.5 md:col-span-2">
                    <label className="text-[10px] font-bold text-slate-400 uppercase ml-2">Transaction Date</label>
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-4">
                   <button type="button" onClick={() => setActiveTab('Sales')} className="flex-1 py-4 border border-slate-300 text-slate-500 font-bold rounded-xl text-xs uppercase tracking-widest hover:bg-slate-50 transition-all">Discard</button>
      ...
      {showPrintPreview && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl p-6 relative">
            <button onClick={() => setShowPrintPreview(null)} className="absolute top-4 right-4 p-2 hover:bg-slate-100 rounded-full"><X size={20} className="text-slate-400" /></button>
            ...
            <div id="bill-content" className="border border-slate-200 rounded-2xl p-6">
                <div className="flex justify-between items-start text-left mb-4">
                    <div><h1 className="text-2xl font-black text-slate-800 m-0 tracking-tight uppercase">Mobile City</h1><p className="text-[9px] text-slate-400 font-bold uppercase m-0">Official Receipt</p></div>
                    <div className="text-right leading-tight"><p className="font-bold text-[11px] m-0 tracking-tight">Hassnain Bhatti</p><p className="text-[10px] m-0 font-mono">0300-6322773</p></div>
                </div>
                <div className="space-y-2 text-left border-y py-4 my-4 border-slate-100">
                    ...
                    <div className="pt-3 space-y-1">
                      ...
                      <div className="flex justify-between text-[11px]"><span className="font-bold text-slate-400 uppercase tracking-tighter">Paid:</span> <span className="font-black text-teal-600">Rs. {Number(showPrintPreview.paidAmount || 0).toLocaleString()}</span></div>
                      <div className="flex justify-between text-[11px] border-t pt-1 mt-1"><span className="font-bold text-slate-400 uppercase">Remaining:</span> <span className="font-black text-red-500">Rs. {Number(showPrintPreview.remainingAmount || 0).toLocaleString()}</span></div>
                    </div>
                </div>
            ...
            <div className="flex gap-2 mt-6">
                <button onClick={() => handlePrint(showPrintPreview)} className="flex-[2] py-3 bg-teal-600 text-white rounded-xl font-bold text-xs uppercase tracking-widest active:scale-95 shadow-xl">Print Invoice</button>
